...
if(isset($_POST['tinh'])){
    ...
    if(isset($_POST['hinh']) && ($_POST['hinh'])=="tgd"){
		...
		$ht->setTen($_POST['ten']);
		$ht->setDodai($_POST['dodai']);
		$str= "Diện tích của hình tam giac dều ".$ht->getTen()." là : ".$ht->tinh_DT()." \n".
				"Chu vi của hình tam giac đều ".$ht->getTen()." là : ".$ht->tinh_CV();
	}
	if(isset($_POST['hinh']) && ($_POST['hinh'])=="tg"){
		$ht=new TamGiacThuong();
		$ht->setTen($_POST['ten']);
		$ht->setCanh($_POST['dodai']);
		$str= "Diện tích của hình tam giac  ".$ht->getTen()." là : ".$ht->tinh_DT()." \n".
				"Chu vi của hình tam giac  ".$ht->getTen()." là : ".$ht->tinh_CV();
	}
		...
		<tr>
			...
			<td><input type="radio" name="hinh" value="hv" 
				...
				<input type="radio" name="hinh" value="tg" 
					<?php if(isset($_POST['hinh'])&&($_POST['hinh'])=="tg") echo 'checked'?>/>Hình tam giác
				<input type="radio" name="hinh" value="cn" 
					<?php if(isset($_POST['hinh'])&&($_POST['hinh'])=="cn") echo 'checked'?>/>Hình chữ nhật
			</td>
		</tr>
		<tr>
			<td>Nhập tên:</td>
